...
func (s *S) TearDownSuite(c *gocheck.C) {
	storage, err := Open("127.0.0.1:27017", "appdb_test")
...
func (s *S) TestOpenCopiesConnection(c *gocheck.C) {
	...
	c.Assert(err, gocheck.IsNil)
	defer storage.session.Close()
	storage2, err := Open("127.0.0.1:27017", "appdb_test")
	c.Assert(err, gocheck.IsNil)
...
func (s *S) TestOpenConnectionRefused(c *gocheck.C) {
	storage, err := Open("127.0.0.1:27018", "appdb_test")
...
func (s *S) TestCollection(c *gocheck.C) {
	storage, _ := Open("127.0.0.1:27017", "appdb_test")
	defer storage.session.Close()
